...
#pragma omp atomic
        ...
        MPI_Reduce(&local_changes, &changes, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
        // 2. Recalculates the centroids: calculates the mean within each cluster
        zeroIntArray(local_pointsPerClass, K);
        zeroFloatMatriz(local_auxCentroids, K, samples);

#pragma omp parallel for private(i, j, class) reduction(+ : local_pointsPerClass[ : K]) reduction(+ : local_auxCentroids[ : K * samples])
        for (i = 0; i < local_lines; i++)
        {
