...
// KERNEL FUNCTION
__global__ void assignPointsToCentroids(float *data, float *centroids, int *classMap, int lines, int samples, int K, int *changes)
{
...
__global__ void updateCentroids(float *data, float *auxCentroids, int *classMap, int *pointsPerClass, int lines, int samples, int K)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void normalizeCentroids(float *auxCentroids, int *pointsPerClass, int samples, int K)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void computeCentroidMovement(float *centroids, float *auxCentroids, float *distCentroids, int samples, int K)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
int main(int argc, char *argv[])
{
    ...
    do
    {
        ...
        // 2. Recalculate the centroids: calculate the mean within each cluster
